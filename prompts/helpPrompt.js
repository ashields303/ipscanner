...
async function prompt() {
  ...
  ------------
  - scan:
  Selecting this option will prompt the user to select a wordlist to use for fuzzing ip addresses for Directory Traversal vulnerabilities.
  Once a wordlist is selected the user is prompted to enter in a list of ip addresses seperated by ','.
  If all the ip's are valid then the scan will be performed. 
  Scan results will be displayed after all ip addresses have been scanned.
  ...
  - targets:
  IIS and NGINX target versions are set by using the 'NGINX_VERSION' and 'IIS_VERSION' located in the '.env'. 
  By default these versions are set to 'Microsoft-IIS/7.0' and 'nginx/1.2.'
  `;
